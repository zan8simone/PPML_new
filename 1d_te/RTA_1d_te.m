%%%
function [RR,TT,AA] = RTA_1d_te(a,L,...
...
% halfnpw        -> truncation radius for Fourier sum                [integer]
...
as(halfnpw+1,1) = -exp(-1i*q(halfnpw+1,halfnpw+1,1)*d(1))*q(halfnpw+1,halfnpw+1,1)/k0^2; 
% incident flux
...
fluxbeg = zeros(L+2,1);
for l = 1:L+2
hx = phi(:,:,l)*(as(:,l) + diag(exp(1i*diag(q(:,:,l)*d(l))))*bs(:,l));
ey =  -A(:,:,l)*(as(:,l) - diag(exp(1i*diag(q(:,:,l)*d(l))))*bs(:,l));

hx = hx.'; ey = ey.';
fluxbeg(l) = real(-hx*ey')/incflux;
end

% Flux at the end of all layers (incl sup & sub)
fluxend = zeros(L+2,1);
for l = 1:L+2
hx = phi(:,:,l)*(diag(exp(1i*diag(q(:,:,l)*d(l))))*as(:,l) + bs(:,l));
ey =  -A(:,:,l)*(diag(exp(1i*diag(q(:,:,l)*d(l))))*as(:,l) - bs(:,l));

hx = hx.'; ey = ey.';
fluxend(l) = real(-hx*ey')/incflux;
end
